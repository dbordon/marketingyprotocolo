...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
    ...
    if ($_GET['act']=='add') {
     ...
     if (isset($_POST['Guardar'])) {
            ...
            if($_POST['estado']!='2doseguimiento'){
                ...
                VALUES ( '$codigo_seg_deser','$codigo_desercion','$motivo', '$obsmotivo','$negociacion','$obsnegociacion','$respuesta',CURDATE( ),'$user','$estado')")
                                                or die('error '.mysqli_error($mysqli));
                                                    ...
                                                    if ($query) {
                                                        header("location: ../../main.php?module=desercionseguimiento&alert=1");   
                                        
                                                    }                                       
          }else{
            $actualizar_desercion="UPDATE desercion set estado='2doseguimiento' WHERE cod_desercion=$codigo_desercion";                               
            $actualizar_ejecutar=mysqli_query($mysqli, $actualizar_desercion);
            
            if ($actualizar_ejecutar) {
                header("location: ../../main.php?module=desercionseguimiento&alert=4");   
            }    
          }
    ...
    elseif ($_GET['act']=='delete') {
        if (isset($_GET['cod_desercion'])) {
           ...
           $query = mysqli_query($mysqli, "UPDATE  desercion SET estado    = '$estado',
                ...
                header("location: ../../main.php?module=desercionseguimiento&alert=3");
            }
        }
    }       
