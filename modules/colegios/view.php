  ...
  <div class="row">
    ...
    elseif ($_GET['alert'] == 3) {
      echo "<div class='alert alert-danger alert-dismissable'>
             ...
             Ya existe el nro de cédula
            </div>";
    }

    ?>
      <h1>Seguimiento a preinscriptos - Visitas a Colegios</h1>
      ...
      <th scope=col>Acción</th>
            ...
            $no = 1;
            if(!empty($id_sede)){
              $query = mysqli_query($mysqli, "SELECT *FROM v_colegios where id_sede =$id_sede;")
              or die('error: '.mysqli_error($mysqli));
            }else {
              $query = mysqli_query($mysqli, "SELECT *FROM v_colegios")
              or die('error: '.mysqli_error($mysqli));
            }
           ...
           $colegio= $data['colegio'];
              ...
              <td width='10' class='center' style='color:#298A08; font-size:20px' >$data[estado]</td>
                      <td class='center' width='1'>
		                <div>
		                  <a data-toggle='tooltip' data-placement='top' title='Realizar Seguimiento' style='margin-right:5px' class='btn btn-primary btn-sm' href='?module=form_colegios&form=edit&id=$data[id_interesado]'>
